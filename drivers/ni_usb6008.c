...
/*
   ...
   Copyright (C) 2011 Prashant Shah, Bernd.Porr@f2s.com
	...
	if (usbduxsub_tmp && usbduxsub_tmp->urbIn) {
		for (i = 0; i < usbduxsub_tmp->numOfInBuffers; i++) {
			...
			dev_dbg(&usbduxsub_tmp->interface->dev,
				"comedi: ni_usb6008: unlinked InURB %d, err=%d\n",
				i, err);
		}
	}
	return err;
...
static int usbduxsub_unlink_OutURBs(struct usbduxsub *usbduxsub_tmp)
{
	...
	if (usbduxsub_tmp && usbduxsub_tmp->urbOut) {
		for (i = 0; i < usbduxsub_tmp->numOfOutBuffers; i++) {
			...
			dev_dbg(&usbduxsub_tmp->interface->dev,
				"comedi: ni_usb6008: unlinked OutURB %d: res=%d\n",
				i, err);
		}
	}
	return err;
			  ...
			  const u8 *firmwareBinary, int sizeFirmware)
{
	...
	if (sizeFirmware > FIRMWARE_MAX_LEN) {
		dev_err(&usbduxsub->interface->dev,
			"ni_usb6008 firmware binary it too large for FX2.\n");
		return -ENOMEM;
	}
...
static int send_dux_commands(struct usbduxsub *this_usbduxsub, int cmd_type)
{
	...
	printk(KERN_DEBUG "comedi%d: ni_usb6008: dux_commands: ",
	       this_usbduxsub->comedidev->minor);
...
static int usbdux_attach(struct comedi_device *dev, struct comedi_devconfig *it)
{
	...
	dev_info(&udev->interface->dev, "comedi%d: attached to ni_usb6008.\n",
		 dev->minor);
...
static int usbdux_detach(struct comedi_device *dev)
{
	...
	if (!dev) {
		printk(KERN_ERR
		       "comedi?: ni_usb6008: detach without dev variable...\n");
		return -EFAULT;
	}
	...
	if (!usbduxsub_tmp) {
		printk(KERN_ERR
		       "comedi?: ni_usb6008: detach without ptr to usbduxsub[]\n");
		return -EFAULT;
	}
